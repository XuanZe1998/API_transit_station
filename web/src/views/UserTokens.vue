<template>
  <div class="tokens-container">
    <div class="action-bar">
      <el-button type="primary" @click="handleAdd">生成新令牌</el-button>
    </div>
...
const fetchTokens = async () => {
  loading.value = true
  try {
    const res = await http.get('/api/user/tokens')
...
const saveToken = async () => {
  try {
    if (form.value.id) {
      await http.put(`/api/user/tokens/${form.value.id}`, form.value)
    } else {
      await http.post('/api/user/tokens', form.value)
    }
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('确定要删除该令牌吗？', '警告', {
    ...
    try {
      await http.delete(`/api/user/tokens/${row.id}`)
